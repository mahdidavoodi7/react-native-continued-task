    ...
    <View style={styles.root}>
      ...
      <ScrollView contentContainerStyle={styles.buttons}>
        <Step
          ...
          onPress={() => {
            void start(EXPORT_PREFIX, 'Export A', false)
            void start(EXPORT_PREFIX, 'Export B', false)
          }}
        />
        <Step
          n={2}
        ...
        <Step
          ...
          detail="Background, cancel in the Live Activity / notification."
          onPress={() => void start(EXPORT_PREFIX, 'Cancellable export', false)}
        />
        <Step
          n={4}
          title="Run, then swipe the app away"
          detail="Relaunch and read the reconcile lines above."
          onPress={() => void start(EXPORT_PREFIX, 'Interruptible export', false)}
        />
        <Step
          n={5}
          title="GPU-gated work"
          detail={`supportsGPU is ${String(ContinuedTasks.supportsGPU)}.`}
          onPress={() => void start(RENDER_PREFIX, 'GPU render', true)}
        />
        <Step
          n={6}
...
const styles = StyleSheet.create({
  root: { flex: 1, paddingTop: 64, paddingHorizontal: 16, backgroundColor: '#fff' },
