...
describe('setPermittedIdentifiers', () => {
  it('expands each prefix to its wildcard form', () => {
    ...
    expect(result[PERMITTED_IDENTIFIERS_KEY]).toEqual(['com.foo.MyApp.export.*'])
  })
  ...
  it('is idempotent across repeated prebuilds', () => {
    const once = setPermittedIdentifiers({} as InfoPlist, ['com.foo.MyApp.export'])
    const twice = setPermittedIdentifiers(once, ['com.foo.MyApp.export'])
    expect(twice[PERMITTED_IDENTIFIERS_KEY]).toEqual(['com.foo.MyApp.export.*'])
  })
  ...
  it('leaves other Info.plist keys alone', () => {
    ...
    expect(setPermittedIdentifiers(existing, ['com.foo.MyApp.export']))
...
describe('setGpuEntitlement', () => {
  it('writes the boolean true Xcode expects', () => {
    expect(setGpuEntitlement({}, true)).toEqual({ [GPU_ENTITLEMENT_KEY]: true })
  })
